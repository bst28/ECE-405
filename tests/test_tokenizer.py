...
def _get_rss_bytes() -> int:
    # Prefer current RSS on Linux from /proc; otherwise fall back to ru_maxrss.
    ...
    # Linux and many Unix platforms report ru_maxrss in kilobytes.
...
def memory_limit(max_mem):
    def decorator(f):
        def wrapper(*args, **kwargs):
            ...
            resource.setrlimit(resource.RLIMIT_AS, (_get_rss_bytes() + max_mem, -1))
            try:
